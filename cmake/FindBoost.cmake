...
if(_FindBoost_CMP0167 STREQUAL "NEW")
  message(
    FATAL_ERROR "The FindBoost module has been removed by policy CMP0167.")
endif()
...
function(_boost_get_existing_target component target_var)
  ...
  if(component MATCHES "^([a-z_]*)(python|numpy)([1-9])\\.?([0-9]+)?$")
    # handle pythonXY and numpyXY versioned components and also python X.Y,
    # mpi_python etc.
    list(
      APPEND
      names
  ...
  # https://github.com/boost-cmake/boost-cmake uses boost::file_system etc. So
  # handle similar constructions of target names
  ...
  foreach(prefix Boost boost)
    foreach(name IN LISTS names)
      if(TARGET "${prefix}::${name}")
        # The target may be an INTERFACE library that wraps around a single
        # other target for compatibility.  Unwrap this layer so we can extract
        # real info.
...
function(_boost_set_legacy_variables_from_config)
  ...
  foreach(comp IN LISTS Boost_FIND_COMPONENTS)
    ...
    # Overwrite if set
    _boost_set_in_parent_scope(Boost_${uppercomp}_FOUND
                               "${Boost_${comp}_FOUND}")
    if(Boost_${comp}_FOUND)
      ...
      elseif(Boost_DEBUG OR Boost_VERBOSE)
        # For projects using only the Boost::* targets this warning can be
        # safely ignored.
        message(
          WARNING
  ...
  if(WIN32)
    _boost_set_if_unset(Boost_LIB_DIAGNOSTIC_DEFINITIONS
...
# -------------------------------------------------------------------------------
# Before we go searching, check whether a boost cmake package is available,
# unless the user specifically asked NOT to search for one.
#
...
if(NOT Boost_NO_BOOST_CMAKE)
  ...
  if(Boost_NO_SYSTEM_PATHS)
    list(APPEND _boost_FIND_PACKAGE_ARGS NO_CMAKE_SYSTEM_PATH
...
#
# _Boost_DEBUG_PRINT_VAR(file line variable_name [ENVIRONMENT] [SOURCE "short
# explanation of origin of var value"])
#
...
function(_Boost_GUESS_COMPILER_PREFIX _ret)
  ...
  elseif(MINGW)
    if(Boost_VERSION_STRING VERSION_LESS 1.34)
      ...
      _boost_compiler_dumpversion(
        ...
        _boost_COMPILER_VERSION_MINOR)
      if(Boost_VERSION_STRING VERSION_GREATER_EQUAL 1.73
         AND _boost_COMPILER_VERSION_MAJOR VERSION_GREATER_EQUAL 5)
  ...
  elseif(UNIX)
    ...
    if(NOT Boost_VERSION_STRING VERSION_LESS 1.69.0)
      ...
      # those compilers, patch is exclude from compiler tag in Boost 1.69+
      # library naming.
  ...
  _boost_debug_print_var(
    "${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "_boost_COMPILER"
    SOURCE "guessed")
  set(${_ret}
      ${_boost_COMPILER}
      PARENT_SCOPE)
...
function(_Boost_COMPONENT_DEPENDENCIES component _ret)
  ...
  #   Boost_VERSION_STRING VERSION_LESS \1\.\2\.\3 AND Boost_VERSION_STRING
  #   VERSION_LESS xxxx);
  #
  ...
  # next. Identical consecutive releases were then merged together by updating
  # the end range of the first block and removing the following redundant
  # blocks.
  #
  ...
  if(Boost_VERSION_STRING)
    ...
    elseif(Boost_VERSION_STRING VERSION_LESS 1.83.0)
      ...
      set(_Boost_IOSTREAMS_DEPENDENCIES regex)
      set(_Boost_JSON_DEPENDENCIES container)
      set(_Boost_LOG_DEPENDENCIES log_setup filesystem thread regex chrono
                                  atomic)
      set(_Boost_MATH_DEPENDENCIES math_c99 math_c99f math_c99l math_tr1
                                   math_tr1f math_tr1l)
      set(_Boost_MPI_DEPENDENCIES serialization)
    ...
    elseif(Boost_VERSION_STRING VERSION_LESS 1.87.0)
      ...
      if(Boost_VERSION_STRING VERSION_GREATER_EQUAL 1.88.0
         AND NOT Boost_NO_WARN_NEW_VERSIONS)
        message(
          WARNING
...
#
# TODO (Re)Check compiler features on (every?) release ??? One may use the
# following command to get the files to check:
#
...
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "_boost_TEST_VERSIONS")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_USE_MULTITHREADED")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_USE_STATIC_LIBS")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_USE_STATIC_RUNTIME")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_ADDITIONAL_VERSIONS")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_NO_SYSTEM_PATHS")
...
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "BOOST_ROOT")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "BOOST_ROOT" ENVIRONMENT)
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "BOOST_INCLUDEDIR")
_boost_debug_print_var(
  "${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "BOOST_INCLUDEDIR"
  ENVIRONMENT)
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "BOOST_LIBRARYDIR")
_boost_debug_print_var(
  "${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "BOOST_LIBRARYDIR"
  ENVIRONMENT)
...
if(NOT Boost_INCLUDE_DIR)
  ...
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "_boost_PATH_SUFFIXES")
...
if(Boost_INCLUDE_DIR)
  ...
  cmake_policy(
    GET CMP0093 _Boost_CMP0093 PARENT_SCOPE # undocumented, do not use outside
                                            # of CMake
  )
  ...
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION")
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION_STRING")
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION_MACRO")
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION_MAJOR")
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION_MINOR")
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION_PATCH")
  _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                         "${CMAKE_CURRENT_LIST_LINE}" "Boost_VERSION_COUNT")
endif()
...
if(NOT DEFINED Boost_LIB_PREFIX)
  # Boost's static libraries use a "lib" prefix on DLL platforms to distinguish
  # them from the DLL import libraries.
  if(Boost_USE_STATIC_LIBS AND ((WIN32 AND NOT CYGWIN) OR GHSMULTI))
...
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_LIB_PREFIX")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "Boost_NAMESPACE")
...
if(Boost_COMPILER)
  ...
  _boost_debug_print_var(
    "${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "_boost_COMPILER"
    SOURCE "user-specified via Boost_COMPILER")
else()
...
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "_boost_MULTITHREADED")
...
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "_boost_RELEASE_ABI_TAG")
_boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}"
                       "${CMAKE_CURRENT_LIST_LINE}" "_boost_DEBUG_ABI_TAG")
...
foreach(COMPONENT ${Boost_FIND_COMPONENTS})
  ...
  if(Boost_${UPPERCOMPONENT}_HEADER_NAME)
    ...
    set(Boost_${UPPERCOMPONENT}_HEADER ON)
    message(
      WARNING "No header defined for ${COMPONENT}; skipping header check "
...
if(Boost_FOUND)
  # The builtin CMake package in Boost 1.70+ introduces a new name for the
  # header-only lib, let's provide the same UI in module mode
  if(NOT TARGET Boost::headers)
    add_library(Boost::headers INTERFACE IMPORTED)
    if(Boost_INCLUDE_DIRS)
      set_target_properties(
        Boost::headers PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                  "${Boost_INCLUDE_DIRS}")
    endif()
  endif()
  ...
  # Supply Boost_LIB_DIAGNOSTIC_DEFINITIONS as a convenience target. It will
  # only contain any interface definitions on WIN32, but is created on all
  # platforms to keep end user code free from platform dependent code.  Also
  # provide convenience targets to disable autolinking and enable dynamic
  # linking.
  ...
  if(WIN32)
    ...
    # the libraries.  If you are linking to a dynamic runtime, then you can
    # choose to link to either a static or a dynamic Boost library, the default
    # is to do a static link.  You can alter this for a specific library
    # "whatever" by defining BOOST_WHATEVER_DYN_LINK to force Boost library
    # "whatever" to be linked dynamically.  Alternatively you can force all
    # Boost libraries to dynamic link by defining BOOST_ALL_DYN_LINK.
